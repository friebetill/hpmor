...
\newcommand{\hpBookTitle}{die Methoden der Rationalität}
\input{hp-titlepage}
\input{hp-contents}
\cleartorecto
\pagestyle{plain}
...
% Revert to normal page style
\pagestyle{headings}
\chapterstyle{evans}% For some reason we need to call this again to have headings on odd pages

\thispagestyle{empty}
\begin{vplace}[.27]
    \itshape\small \parindent=0pt Unter dem Mondlicht schimmert ein winziges
    Silberfragment, ein Bruchteil einer Linie{\el}

    \medskip
    (fallende schwarze Roben)

    \medskip
    {\el} Blut schwappt literweise heraus, und jemand schreit ein Wort.
\end{vplace}
\mainmatter
